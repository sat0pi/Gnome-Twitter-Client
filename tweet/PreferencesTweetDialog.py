...
class PreferencesTweetDialog(gtk.Dialog):
    __gtype_name__ = "PreferencesTweetDialog"
    prefernces = {}
...
def NewPreferencesTweetDialog():
    """NewPreferencesTweetDialog - returns a fully instantiated
    PreferencesTweetDialog object. Use this function rather than
    creating a PreferencesTweetDialog instance directly.
    """

    #look for the ui file that describes the ui
    ui_filename = os.path.join(getdatapath(), 'ui', 'PreferencesTweetDialog.ui')
    if not os.path.exists(ui_filename):
        ui_filename = None

    builder = gtk.Builder()
    builder.add_from_file(ui_filename)
    dialog = builder.get_object("preferences_tweet_dialog")
    dialog.finish_initializing(builder)
    return dialog

if __name__ == "__main__":
    dialog = NewPreferencesTweetDialog()
    dialog.show()
    gtk.main()
